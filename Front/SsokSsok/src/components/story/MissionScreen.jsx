...
const MissionScreen = ({ storyData, assets }) => {
  ...
  useEffect(() => {
    const fetchMission = async () => {
      if (!storyData?.instructions) {
        setMissionText("지시사항이 없습니다.");
        return;
      }

      try {
        const missionUrl = assets[storyData.instructions];
        if (!missionUrl) {
  ...
  return (
    ...
    >
      ...
      <div className="text-center font-cafe24">
        ...
        <p className="mt-2 text-lg font-medium whitespace-pre-line">{missionText}</p>
      </div>
