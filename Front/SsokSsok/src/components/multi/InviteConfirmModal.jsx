...
const InviteConfirmModal = ({ friend, nickname, onConfirm, onClose }) => {
  const handleInvite = async () => {
    try {
      const res = await sendInviteApi(friend); // friend = 초대 요청 보낼 상대의 아이디

      if (res.data.isSuccess) {
        const roomId = res.data.data.roomId;
        alert(`${friend}님께 초대를 보냈습니다!`);
        onConfirm(roomId); // roomId 콜백으로 넘기기
      } else {
    ...
    <div className="modal-wrapper">
      <div className="relative w-[70rem]">
        ...
        <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center px-10">
          ...
          <h2 className="modal-title mt-20 text-center leading-relaxed">
