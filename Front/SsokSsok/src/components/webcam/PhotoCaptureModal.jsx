...
const PhotoCaptureModal = ({ isOpen, previewUrl, onSave, onClose, duration = 10000 }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="fixed top-0 left-0 w-full h-[100%] bg-black bg-opacity-40 flex items-center justify-center z-[9999]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          ...
          >
            ...
            {/* 콘텐츠 */}
            <div className="absolute w-[80%] h-[35%] flex items-center justify-center flex-col gap-4">
              <h2 className="text-4xl font-bold text-black text-center font-cafe24 mb-1">기록 완료!</h2>
              <p className="text-xl font-bold text-black font-cafe24">촬영한 사진은 나만의 앨범에서 모아 볼 수 있어요!</p>

              {/* 캡처 이미지 */}
              <img
                src={previewUrl}
                alt="캡처 미리보기"
                className="max-w-full max-h-full rounded-xl border-4 border-white shadow-xl"
              />

              {/* <p className="text-2xl font-bold text-black font-cafe24">내 사진책에서 볼 수 있어요!</p> */}

              {/* 저장 버튼 이미지 */}
              <button onClick={onSave} className="relative w-52 mt-1 hover:scale-105 transition">
